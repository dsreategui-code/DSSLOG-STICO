...
def _proceso_vehiculo(env: "simpy.Environment", veh_id: str, ruta: List[int],
    ...
    mu = -0.5 * ctx.sigma_viaje ** 2          # asegura E[multiplicador] = 1
    prev = ruta[0]                            # HUB (0)
    for node in ruta[1:]:
        viaje = float(ctx.tiempo_min[prev][node]) * rng.lognormvariate(mu, ctx.sigma_viaje)
        if rng.random() < ctx.incid_prob[node]:
            viaje += ctx.incid_delay_min[node]
...
def simular_una_corrida(ctx: ContextoSim, seed: int = 0) -> List[dict]:
    ...
    for veh_id, ruta in ctx.rutas.items():
        ...
        env.process(_proceso_vehiculo(env, veh_id, seq, ctx, rng, registros))
    env.run()
    return registros
...
def contexto_desde_resultado(resultado: dict, modelo, *,
                             ...
                             sigma_viaje: float = 0.25) -> ContextoSim:

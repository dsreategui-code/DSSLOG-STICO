...
from utils.constants import VISTA_SUPERVISOR_CONFIG, VISTA_DEMO_ROLE_SELECTION
...
def _cargar_dataset():
    ...
    if res.ok:
        ...
        log_bitacora("Supervisor - carga de dataset", f"{n_p} pedidos · {n_v} vehiculos")


def render():
    render_view_title(
        "Carga de datos",
        "Revisa el dataset operativo del dia. Genera o carga los archivos para iniciar "
        "la jornada simulada con tu flota."
    )
    ...
    with col1:
        if primary("Generar dataset sintetico", key="sup_gen_ds",
                   use_container_width=True):
            if _generar_dataset():
                st.success("Dataset generado correctamente.")
                log_bitacora("Supervisor - generacion de dataset", f"Ruta: {DEMO_DATASET_DIR}")
                st.rerun()
    with col2:
        if primary("Cargar dataset", key="sup_load_ds",
    ...
    with col_l:
        if secondary("Cambiar de rol", key="sup_data_back",
                     use_container_width=True):
            st.session_state.vista = VISTA_DEMO_ROLE_SELECTION
            st.rerun()
    with col_r:
        if primary("Configuracion", key="sup_data_next",
                   use_container_width=True,
                   disabled=not st.session_state.get("dataset_validado")):
            navigate_to(VISTA_SUPERVISOR_CONFIG)
